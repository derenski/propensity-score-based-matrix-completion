...
durationProbToConditionalPropScore <- function(reversedTreatmentDurationProbMatrix){
    
    zeroDurationProb <- reversedTreatmentDurationProbMatrix[, dim(reversedTreatmentDurationProbMatrix)[2]]

    nonZeroDurationProbs <- reversedTreatmentDurationProbMatrix[, 1:(dim(reversedTreatmentDurationProbMatrix)[2]-1)]

    finalCondProbMat <- nonZeroDurationProbs/(nonZeroDurationProbs+t(sapply(zeroDurationProb, FUN=rep, times=dim(reversedTreatmentDurationProbMatrix)[2]-1)))
    
    return(finalCondProbMat)
}


naiveWeighedPropensityEstimator <- function(Y, D, treatmentDurationProbMatrix){ ### YATTA!!!!!!!!

    finalCondProbMat <- durationProbToConditionalPropScore(treatmentDurationProbMatrix)
    
    numTimePoints <- dim(Y)[2]
     
    allDeltas <- rowSums(D)
    
    uniqueDeltas <- sort(unique(allDeltas))
    
    positiveDeltas <- uniqueDeltas[2:length(uniqueDeltas)]

    treatmentTrajectories <- list()
    
    for (d in positiveDeltas){
        
        unitSample <- allDeltas %in% c(0, d)
        
      #  prob0 <- sum(allDeltas==0)/length(allDeltas)
        
      #  probd <- sum(allDeltas==d)/length(allDeltas)
        
      #  sampleScalingFactor <- 1/(prob0+probd)
        
        YThisTime <- Y[unitSample, ]
        
        thePropMatThisD <- array(0, dim=dim(YThisTime))
        
        DforThisLength <- D[unitSample, ]
        
        probDuration <- finalCondProbMat[unitSample, 
                                        (dim(finalCondProbMat)[2]-d+1)]
        
        thePropMatThisD[, (dim(thePropMatThisD)[2]-d+1):dim(thePropMatThisD)[2]] <- probDuration
        
        #scoreOne <- array(1, dim=dim(thePropMatThisD))
        
        #scoreOne <- apply(scoreOne , MARGIN=2, FUN=function(x) x/mean(x))
        scoreOne <- 1
        scoreOne[is.infinite(scoreOne)] <- 1
        scoreOne[is.na(scoreOne)] <- 1

        scoreTwo <- (thePropMatThisD/(1-thePropMatThisD))
        #scoreTwo <- apply(scoreTwo , MARGIN=2, FUN=function(x) x/mean(x))
        scoreTwo[is.infinite(scoreTwo)] <- 1
        scoreTwo[is.na(scoreTwo)] <- 1
         
        treatedWeightedYs <- YThisTime*DforThisLength*(scoreOne)
    
        untreatedWeightedYs <- YThisTime*(1-DforThisLength)*(scoreTwo)

        estimatedEffect <- (colMeans(treatedWeightedYs)-colMeans(untreatedWeightedYs))/colMeans(thePropMatThisD)

        treatmentTrajectories <- c(treatmentTrajectories, 
                                   list(estimatedEffect[(numTimePoints-d+1):numTimePoints]))

    }
    
    bigNAMat <- array(NA, dim=c(length(treatmentTrajectories), max(lengths(treatmentTrajectories))))     
                                     
    for (index in 1:length(treatmentTrajectories)){
    
        bigNAMat[index, 1:lengths(treatmentTrajectories)[index] ] <- treatmentTrajectories[[index]]
    
    }
    
         numberOfEachUnitType <- table(rowSums(D))
...
parameterMakerStaggered <- function(N, Time=20, propTreat=.2, generatingModel='Bin', K, 
  ...
  fullyParametricProbModel <- function(N=N, Time=Time, generatingModel='Poi', propTreat=propTreat, K, theBeta,
     ...
     discreteUniform <- function(x, p, deltaMax){ ## Delta | X, T is uniform
    
        leftLimit <- max(1, floor(p*deltaMax))

        rightLimit <- deltaMax

        theIndices <- Reduce('|', list(x < leftLimit, x > rightLimit))

        return(c(1/(rightLimit-leftLimit+1), 0)[theIndices+1])
    
    }

    truncatedShiftedGeometric <- function(x, p, deltaMax){ ## Delta | X, T is uniform

        valueStores <- rep(0, length(x))

        possibleVals <- p^seq(1, deltaMax, 1)

        possibleVals <- possibleVals/sum(possibleVals)

        theIndices <- which(!Reduce('|', list(x < 1, x > deltaMax)))

        valueStores[theIndices] <- possibleVals[theIndices]
    ...
    deltaGenerator <- function(n, densityFunctionForPositiveDelta, densityFunctionParameters=list(p=.5, deltaMax=10)){

        p <- densityFunctionParameters[['p']]

        deltaMax <- densityFunctionParameters[['deltaMax']]

        allDensityValues <- c(1-p, p*densityFunctionForPositiveDelta(1:deltaMax, p=p, deltaMax=deltaMax))

        randomSample <- sample(0:deltaMax, size=n, replace=TRUE, prob = allDensityValues)
    ...
    rFunPoi <- function(r, xBeta, C, lengthCutoff){
    ...
    rFunOther <- function(r, xBeta, C, lengthCutoff){
      
      vals <- 1-probFun(r=r, xBeta=xBeta)
    ...
    if (generatingModel=='Bin'){
        ...
        r <- uniroot(rFunBin, xBeta= XsforEstimation %*% theBeta, C=propTreat,
        ...
        probDurations <- t(sapply(actualProbTreat, FUN=function(x) dbinom(seq(0, Time, 1),       
                                                                          size=lengthCutoff, prob=x)))
    
    }else if (generatingModel=='Unif'){
        
        r <- uniroot(rFunOther, xBeta= XsforEstimation %*% theBeta, C=propTreat,
                 lengthCutoff=lengthCutoff,
                 lower=-500, upper=500)$root
        
        actualProbTreat <- probFun(r=r, xBeta=Xs %*% theBeta)

        treatmentLengths <- sapply(actualProbTreat, FUN=function(x) deltaGenerator(n=1, densityFunctionForPositiveDelta=discreteUniform, 
                                           densityFunctionParameters=list(p=x, deltaMax=lengthCutoff)))                           

        probDurations <- t(sapply(actualProbTreat, FUN=function(x) c(1-x, x*discreteUniform(1:Time, p=x, 
                                                                                                            deltaMax=lengthCutoff))))
    }else if (generatingModel=='Geo'){
        ...
        treatmentLengths <- sapply(actualProbTreat, FUN=function(x) deltaGenerator(n=1, densityFunctionForPositiveDelta=truncatedShiftedGeometric, 
                                           densityFunctionParameters=list(p=x, deltaMax=lengthCutoff)))                           

        probDurations <- t(sapply(actualProbTreat, FUN=function(x) c(1-x, x*truncatedShiftedGeometric(1:Time, p=x, 
                                                                                                            deltaMax=lengthCutoff))))
    }else if (generatingModel=='Poi'){

      r <- uniroot(rFunPoi, xBeta= XsforEstimation %*% theBeta, C=propTreat,
                 lengthCutoff=lengthCutoff,
                 lower=-500, upper=500)$root
      ...
      probDurations <- t(sapply(lambdaParams, FUN=function(x) dpois(seq(0, Time, 1), lambda=x)))
      
    }else{ 
    
        stop("Chosen generating model not available, choose: \'Bin\', \'Unif\', \'Geo\', or \'Poi\'")
    ...
    trueTreatDurationProbs <- probDurations
                                
    truePropScoreMat <- plyr::aaply(trueTreatDurationProbs, 1, function(x)cumsum(rev(x)))[, 1:Time]
  ...
  #  truePropScoreMat <- sapply(1:N, FUN=function(x) c(rep(0, Time-lengthCutoff),
   #                                                rep(dbinom(treatmentLengths[x], size=lengthCutoff, prob=actualProbTreat[x]), lengthCutoff)))
    ...
    finalOutput <- list(trueTreatDurationProbs, truePropScoreMat, caseDurationData, eventAtAndAfterIndicator, covArray)
    
    names(finalOutput) <- c("trueTreatDurationProbs", "truePropScoreMat", "caseDurationData", "W", "covArray")
  ...
  trueTreatDurationProbs <- simulatedDataPack$trueTreatDurationProbs
  
  covArray <- simulatedDataPack$covArray
  
  if (generatingModel=='Poi'){
  ...
  finalStuff <- list(W, propScoreEst, trueTreatDurationProbs , trueWeightMatrix, perturbedFinalWeightMatrix, weightMatrixConditionalEst, covArray)
  
  names(finalStuff) <- c("W", "propScoreEst", "trueTreatDurationProbs", "trueWeightMatrix", "weightMatrixEst", "weightMatrixConditionalEst", "covArray")
